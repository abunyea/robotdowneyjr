...
let rec ids player initial = 
	...
	let rec ids_helper() =
		...
		match path with
			| [] -> incr depth; (prerr_endline (string_of_int !depth)); Stack.push (initial.board, []) q; ids player initial;
			| x::t -> (continuation:= true); (move_sequence:= t); x);;
...
let check_sequence state player sequence = 
	...
	let check_validity (x1, y1, x2, y2, _, _) state =
		true in
		(* let board = state.board in 
		((board.(y1).(x1) = player) && (board.(y2).(x2) = Empty)) in *)
