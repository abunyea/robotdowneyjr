...
let hernandez_bot state player = 
  ...
  let move_set = available_moves state.board player in
	...
	let mustache_dist (x2,y2) sum (x,y) =
	...
	let improve board move =
			...
			let original = (dist mustache_dist board) in
			original -. (dist mustache_dist resulting_board) in
...
let mustache_bot = hernandez_bot 
