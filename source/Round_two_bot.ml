...
let num_moves_dif player state =
	...
	if has_won board player then cWINNING_SCORE else
...
let our_eval player state = 
	let score = (num_moves_dif player state) +. (furthest_back player state) in
	if player = P1 then score else (-1.0 *. score)
...
let round_two_bot = build_minimax_bot our_eval 3;;
